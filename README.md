...
`CPA_BASE_URL` is only used by the server to call CPA, so it can be a service hostname or private network address. It is not used for browser navigation or frame trust.
