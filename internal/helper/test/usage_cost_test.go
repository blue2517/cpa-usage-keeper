...
func TestCalculateUsageTokenCostBreakdownClaudePricingFallsBackToCachedTokens(t *testing.T) {
	pricing := entities.ModelPriceSetting{
		PricingStyle:            entities.ModelPricingStyleClaude,
		PromptPricePer1M:        1.5,
		CompletionPricePer1M:    9,
		CachePricePer1M:         0.15,
		CacheCreationPricePer1M: 1,
	}
	breakdown := helper.CalculateUsageTokenCostBreakdown(helper.UsageTokenCostInput{
		InputTokens:  28_054,
		OutputTokens: 61,
	...
	wantInput := (float64(28_054-24_471) / 1_000_000.0) * 1.5
	wantOutput := (float64(61) / 1_000_000.0) * 9.0
	wantCached := (float64(24_471) / 1_000_000.0) * 0.15

	assertCostClose(t, breakdown.InputCostUSD, wantInput)
	assertCostClose(t, breakdown.OutputCostUSD, wantOutput)
	assertCostClose(t, breakdown.CachedCostUSD, wantCached)
	assertCostClose(t, breakdown.TotalCostUSD, wantInput+wantOutput+wantCached)
}

func TestCalculateUsageTokenCostBreakdownClaudePricingPrefersExplicitCacheFields(t *testing.T) {
	pricing := entities.ModelPriceSetting{
		PricingStyle:            entities.ModelPricingStyleClaude,
		PromptPricePer1M:        10,
		CompletionPricePer1M:    20,
